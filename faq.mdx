...
<AccordionGroup>
  <Accordion title="Do I need to keep my computer on?">
    ...
    ```bash
    pm2 start "remoteagentchat start" --name my-project --cwd /path/to/project
    pm2 startup
    pm2 save
    ```
  </Accordion>
  ...
  <Accordion title="Can I use multiple projects?">
    Yes. Run `remoteagentchat init` in each project directory to register a new agent. Each agent gets a unique ID and appears as a separate entry in the bot and dashboard.
  ...
  <Accordion title="What if my agent goes offline?">
    ...
    To bring the agent back online, run `remoteagentchat start` in your project directory, or by name: `remoteagent start "project-name"`.
  ...
  <Accordion title="Does RemoteAgent work on Windows?">
    The agent is supported on Windows via **WSL 2** (Windows Subsystem for Linux). Install WSL 2, then install Node.js inside the WSL environment and run `remoteagentchat` from there.
