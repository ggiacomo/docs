...
<AccordionGroup>
  ...
  <Accordion title="Is my API key safe?">
    Yes. Your API key is stored in `~/.remoteagent/config.json` on your local machine and is **never transmitted to RemoteAgent servers**. The agent calls the AI model API directly from your machine. RemoteAgent only sees the prompt you sent via Telegram, not the full conversation context or your credentials.
