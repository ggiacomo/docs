...
```bash
remoteagentchat init --runner custom --runner-bin /path/to/your/binary
```
...
remoteagentchat init --runner custom --runner-bin /usr/local/bin/my-ai-wrapper
```
...
remoteagentchat init --runner custom --runner-bin /usr/local/bin/openai-runner.py
```
...
If your binary needs environment variables (API keys, config paths), set them in the shell environment where you run `remoteagentchat start`. They are inherited by child processes:
