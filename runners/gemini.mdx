...
After authenticating, credentials are cached locally. The agent uses these cached credentials when invoking the Gemini CLI — no API key needs to be entered during `remoteagentchat init`.
