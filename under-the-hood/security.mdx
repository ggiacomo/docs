...
These files are written by `remoteagentchat init` and read only by the local agent process. They are never uploaded, synced, or referenced by any network request made by the relay.
