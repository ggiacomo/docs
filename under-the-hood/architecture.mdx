---
title: "Architecture"
description: "How RemoteAgent works — the message flow from Telegram to your agent and back."
---

RemoteAgent is built on a serverless-first architecture. There is no custom server to maintain on the cloud side, and no persistent connection between Telegram and your machine. Everything is event-driven.

## Components

**RemoteAgent cloud** — a serverless web app that handles Telegram webhook events, authenticates requests, and relays messages. It has no persistent state beyond what's in the database.
...
**Your API key never leaves your machine.** The agent calls your AI runner locally. No credentials are transmitted to RemoteAgent servers.
