...
Run remoteagentchat start in your project directory to bring it back.
```
...
<CodeGroup>
  ```bash pm2
  pm2 start "remoteagentchat start" --name my-api --cwd /path/to/project
  pm2 save
  pm2 startup
  ...
  ```bash systemd
  # Create /etc/systemd/system/remoteagentchat-myapi.service
  # (see the Multi-project page for the full unit file)
  systemctl enable remoteagentchat-myapi
  systemctl start remoteagentchat-myapi
  ```
</CodeGroup>
