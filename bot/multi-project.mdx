...
Each agent is initialized in its own project directory. Navigate to the second project and run `remoteagentchat init`:
...
```bash
remoteagentchat start "my-api"
remoteagentchat start "frontend"
```

The name matches the project name you assigned during `remoteagentchat init`.
...
- **All agents offline** — the bot notifies you that no agents are reachable and suggests running `remoteagentchat start`.
...
<CodeGroup>
  ```bash systemd (Linux)
  # /etc/systemd/system/remoteagentchat-myapi.service
  [Unit]
  ...
  ExecStart=remoteagentchat start
  Restart=on-failure
  RestartSec=5
  ...
  pm2 start "remoteagentchat start" --name my-api --cwd /path/to/my-api
  pm2 start "remoteagentchat start" --name frontend --cwd /path/to/frontend
  pm2 save
  pm2 startup
  ```
</CodeGroup>
